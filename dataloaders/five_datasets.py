...
def get(data_path, args, seed=0,fixed_order=False,pc_valid=0.05, samples_per_task=-1):
    data={}
    taskcla=[]
    size=[3,32,32]

    idata=np.arange(5)
    # idata=np.array([3,1,2,0,4])
    # if not fixed_order:
    ...
    if not os.path.isdir('./data/Five_data/binary_mixture_5_Data/'):
        ...
        for n,idx in enumerate(idata):
            if idx==0:
                ...
                dat={}
                dat['train']=datasets.CIFAR10(args.data_path,train=True,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.CIFAR10(args.data_path,train=False,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n]={}
                data[n]['name']='cifar10'
                data[n]['ncla']=10
                for s in ['train','test']:
            ...
            #             data[n][s]['y'].append(target.numpy()[0])
                ...
                std=(0.2752,)
                dat={}
                dat['train']=datasets.MNIST(args.data_path,train=True,download=True,transform=transforms.Compose([
                    transforms.Pad(padding=2,fill=0),transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.MNIST(args.data_path,train=False,download=True,transform=transforms.Compose([
                    transforms.Pad(padding=2,fill=0),transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n]={}
                data[n]['name']='mnist'
                data[n]['ncla']=10
                ...
                dat = {}
                dat['train']=datasets.SVHN(args.data_path,split='train',download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.SVHN(args.data_path,split='test',download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n] = {}
                data[n]['name']='svhn'
                data[n]['ncla']=10
                for s in ['train','test']:
                ...
                std=(0.3318,)
                dat={}
                dat['train']=FashionMNIST(args.data_path, train=True, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                dat['test']=FashionMNIST(args.data_path, train=False, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                data[n]={}
            ...
            #             data[n][s]['y'].append(target.numpy()[0])
                ...
                std=(0.4501,)
                dat={}
                dat['train']=notMNIST(args.data_path, train=True, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=notMNIST(args.data_path, train=False, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                #mean, std = utils.compute_mean_std_dataset(dat['train']); print(mean,std); sys.exit()
                data[n]={}
    ...
    return data, taskcla,size, n_tasks, data['ncla']
...
class notMNIST(torch.utils.data.Dataset):
    ...
    def __init__(self, root, train=True,transform=None, download=False):
        self.root = os.path.expanduser(root)
        self.transform = transform
        self.filename = "notmnist_train.pkl"
