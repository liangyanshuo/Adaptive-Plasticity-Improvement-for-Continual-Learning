...
class Net(nn.Module):
    ...
    def observe(self, x, y, t, evalue = False, writer=None):
        if t != self.current_task:
            if self.arch == 'alexnet' and t>0:
                ...
                if len(self.Grad_Norm[0]):
                    avg = []
                    for k in range(5):
                        avg.append(sum(self.Grad_Norm[k])/len(self.Grad_Norm[k]))
                    writer.add_scalar(f"5.AVG-Grad-Norm", sum(avg)/len(avg), self.current_task)

                    for KK in self.Grad_Norm.keys():
                        self.Grad_Norm[KK] = []
            self.current_task = t
            self.iter = 0
            ...
            if self.arch == 'mlp' and writer != None:
                ...
                for k, (m,params) in enumerate(self.net.named_parameters()):
                    if k<15 and len(params.size())!=1:
                        per_layer_norm.append(params.grad.norm(p=2))
                        kk +=1
            elif self.arch == 'alexnet1' and writer != None:
                kk = 0
                per_layer_norm = []
                for k, (m,params) in enumerate(self.net.named_parameters()):
                    if k<7 and len(params.size())!=1:
                        per_layer_norm.append(params.grad.norm(p=2))
                        kk +=1
                ...
                for k, (m,params) in enumerate(self.net.named_parameters()):
                    if k<4 and len(params.size())!=1:
                        per_layer_norm.append(params.grad.norm(p=2))
                        kk +=1
            elif self.arch == 'resnet' and writer != None:
                kk = 0 
                per_layer_norm = []
                for k, (m,params) in enumerate(self.net.named_parameters()):
            ...
            if self.arch == 'mlp' and writer != None:
                ...
                for k, (m,params) in enumerate(self.net.named_parameters()):
                    if k<15 and len(params.size())!=1:
                        ...
                        # else:
                            self.Grad_Norm[kk].append(params.grad.norm(p=2).item())
                        kk +=1
            elif self.arch == 'alexnet1' and writer != None:
                kk = 0
                for k, (m,params) in enumerate(self.net.named_parameters()):
    ...
    def update_threshold(self, task_id):
        if 'alexnet' == self.arch:
            # self.threshold = np.array([self.args.thres] * 5) + task_id*np.array([(1-self.args.thres)/self.n_tasks] * 5)
            self.threshold = np.array([0.97] * 5) + task_id*np.array([0.03/self.n_tasks] * 5)
        elif 'lenet' == self.arch:
            self.threshold = np.array([0.98] * 5) + task_id*np.array([0.02/self.n_tasks] * 5)
