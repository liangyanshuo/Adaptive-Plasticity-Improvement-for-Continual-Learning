...
def life_experience(model, data, ids, args, logger):
    ...
    for i, t in enumerate(ids):
        ...
        if args.model in ['trgp']:
            model.define_begin(xtrain, ytrain, t)
        ...
        for ep in prog_bar:
            ...
            for bi in range(0, len(idx), args.batch_size):
                ...
                # ipdb.set_trace()
                if args.model not in ['hat', 'pat']:
                    loss = model.observe(v_x, v_y, t, writer=writer)
                else:
                    loss = model.observe(v_x, v_y, t, float(bi)/len(idx), writer)
...
if __name__ == "__main__":
    ...
    if args.model in ['gpm', 'dualgpm']:
        path = os.path.join('Results', 'enabled_{}/dataset_{}/arch_{}/size_{}/method_{}'.format(args.cuda_enabled, args.dataset, args.model_arch, args.model_size, args.model))
    else:
        raise NotImplementedError
    args.path = path
