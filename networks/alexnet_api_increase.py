...
class Learner(nn.Module):
    ...
    def forward(self, x, t=0):
        ...
        if t != 0:
            x = torch.cat([x] + [F.pad(F.conv2d(x, self.weight1[i].transpose(1,0).view(-1,x.shape[1],self.ksize[0],self.ksize[0])),(0,3,0,3), 'replicate') for i in range(t)] + [torch.zeros(x.size(0),sum(self.expand[0][t:]), x.size(2), x.size(3), device=x.device)], dim=1)
        else:
        ...
        if t != 0:
            x = torch.cat([x] + [F.pad(F.conv2d(x, self.weight2[i].transpose(1,0).view(-1,x.shape[1],self.ksize[1],self.ksize[1])),(0,2,0,2), 'replicate') for i in range(t)] + [torch.zeros(x.size(0),sum(self.expand[1][t:]), x.size(2), x.size(3), device=x.device)], dim=1)
        else:
        ...
        if t != 0:
            x = torch.cat([x] + [F.pad(F.conv2d(x, self.weight3[i].transpose(1,0).view(-1,x.shape[1],self.ksize[2],self.ksize[2])),(0,1,0,1), 'replicate') for i in range(t)] + [torch.zeros(x.size(0),sum(self.expand[2][t:]), x.size(2), x.size(3), device=x.device)], dim=1)
        else:
