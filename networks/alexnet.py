...
#         self.conv3 = torch.nn.Conv2d(128, 256, kernel_size=2, bias=False)
#         if self.args.use_track:
#             self.bn3 = torch.nn.BatchNorm2d(256, momentum=0.1)
#         else:
#             self.bn3 = torch.nn.BatchNorm2d(256, track_running_stats=False)
...
#         self.fc1 = torch.nn.Linear(256 * s * s, 2048, bias=False)
#         if self.args.use_track:
#             self.bn4 = torch.nn.BatchNorm1d(2048, momentum=0.1)
#         else:
#             self.bn4 = torch.nn.BatchNorm1d(2048, track_running_stats=False)

#         self.fc2 = torch.nn.Linear(2048, 2048, bias=False)
#         if self.args.use_track:
#             self.bn5 = torch.nn.BatchNorm1d(2048, momentum=0.1)
#         else:
#             self.bn5 = torch.nn.BatchNorm1d(2048, track_running_stats=False)
...
#             h = self.maxpool(self.drop2(self.relu(self.bn3(h))))
#             h = h.reshape(x.size(0), -1)
#             y.append(h)
...
#                 h = self.maxpool(self.drop1(self.relu(self.bn2(self.F_conv(self.conv2, h, vars[1])))))
#                 h = self.maxpool(self.drop2(self.relu(self.bn3(self.F_conv(self.conv3, h, vars[2])))))

#                 h = h.reshape(x.size(0), -1)

#                 h = self.drop2(self.relu(self.bn4(F.linear(h, vars[3]))))
#                 h = self.drop2(self.relu(self.bn5(F.linear(h, vars[4]))))
...
#                 try:
#                     h = self.F_batch_norm(h, weight=vars[1], bias=vars[2], bn=self.bn1)
#                 except:
#                     ipdb.set_trace()
#                 h = self.maxpool(self.drop1(self.relu(h)))

#                 h = self.F_conv(self.conv2, h, vars[3])
#                 h = self.F_batch_norm(h, weight=vars[4], bias=vars[5], bn=self.bn2)
#                 h = self.maxpool(self.drop1(self.relu(h)))

#                 h = self.F_conv(self.conv3, h, vars[6])
#                 h = self.F_batch_norm(h, weight=vars[7], bias=vars[8], bn=self.bn3)
#                 h = self.maxpool(self.drop2(self.relu(h)))

#                 h = h.reshape(x.size(0), -1)

#                 h = F.linear(h, vars[9])
#                 h = self.F_batch_norm(h, weight=vars[10], bias=vars[11], bn=self.bn4)
#                 h = self.drop2(self.relu(h))

#                 h = F.linear(h, vars[12])
#                 h = self.F_batch_norm(h, weight=vars[13], bias=vars[14], bn=self.bn5)
#                 h = self.drop2(self.relu(h))

#                 y = F.linear(h, vars[15])
#         else:
#             h = self.maxpool(self.drop1(self.relu(self.bn1(self.conv1(x)))))
#             h = self.maxpool(self.drop1(self.relu(self.bn2(self.conv2(h)))))
#             h = self.maxpool(self.drop2(self.relu(self.bn3(self.conv3(h)))))
#             h = h.reshape(x.size(0), -1)
#             try:
#                 h = self.drop2(self.relu(self.bn4(self.fc1(h))))
#             except:
#                 ipdb.set_trace()
#             h = self.drop2(self.relu(self.bn5(self.fc2(h))))
#             y = self.head(h)
...
#     def F_batch_norm(self, x, weight, bias, bn):
